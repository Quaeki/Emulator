...
class ShellEmulatorGUI:
    ...
    def __init__(self, root: tk.Tk, vfs_path: Optional[str], startup_script: Optional[str]):
        ...
        self.println("Добро пожаловать в эмулятор оболочки (Вариант №18, Этап 4).")
        self.println("Команды: ls [path], cd <path>, date, tac <path>, vfs-info, pwd, exit.")
        self.println("Пути: поддерживаются абсолютные (/a/b) и относительные (../b).")
        self.println()
...
def parse_args():
    p = argparse.ArgumentParser(description="Эмулятор оболочки — Вариант №18, Этап 4")
